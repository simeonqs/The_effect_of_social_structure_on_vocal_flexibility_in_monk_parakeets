...
# Description: This script preps the repertoire data for analysis.  
...
# Summarise rep size per ind
rep_size = data.frame(ind = inds_enough)
rep_size$n = sapply(rep_size$ind, function(ind) length(unique(st_20$`call type`[st_20$bird == ind])))
rep_size$n_calls = sapply(rep_size$ind, function(ind) length(st_20$`call type`[st_20$bird == ind]))
rep_size$n_recs = sapply(rep_size$ind, function(ind) length(unique(st_20$file[st_20$bird == ind])))
rep_size$entropy = vapply(rep_size$ind, function(ind) calc.ent(st_20$`call type`[st_20$bird == ind]), 
                          numeric(1))

# Plot relative frequencies per individual
pdf(path_pdf_relative_frequencies_20)
par(mfrow = c(2, 2))
for(ind in rep_size$ind){
  sub = st_20[st_20$bird == ind,]
  rfs = vapply(unique(st_20$`call type`), function(ct) 
    length(which(sub$`call type` == ct)), numeric(1))/nrow(sub)
  barplot(rfs, ylim = c(0, 1), xaxt = 'n', main = ind)
}
dev.off()
...
master_dat = data.frame(ind = unique(c(rep_size$ind, inds_age, inds_sex, inds_nesting)))

# Add rep sizes
master_dat = merge(master_dat, rep_size, by = 'ind', all.x = T, all.y = F)
...
# Summarise rep size per ind
rep_size = data.frame(ind = inds_enough)
rep_size$n = sapply(rep_size$ind, function(ind) length(unique(st_21$`call type`[st_21$bird == ind])))
rep_size$n_calls = sapply(rep_size$ind, function(ind) length(st_21$`call type`[st_21$bird == ind]))
rep_size$n_recs = sapply(rep_size$ind, function(ind) length(unique(st_21$file[st_21$bird == ind])))
rep_size$entropy = vapply(rep_size$ind, function(ind) calc.ent(st_21$`call type`[st_21$bird == ind]), 
                          numeric(1))

# Plot relative frequencies per individual
pdf(path_pdf_relative_frequencies_21)
par(mfrow = c(2, 2))
for(ind in rep_size$ind){
  sub = st_21[st_21$bird == ind,]
  rfs = vapply(unique(st_21$`call type`), function(ct) 
    length(which(sub$`call type` == ct)), numeric(1))/nrow(sub)
  barplot(rfs, ylim = c(0, 1), xaxt = 'n', main = ind)
}
dev.off()
...
save(master_dat_20, master_dat_21, file = path_master_dat_rep_size)
